...
function makeSignature() {
  ...
  const hmac = require('crypto').createHmac('sha256', SOLAPI_SECRET);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Solapi request:', JSON.stringify(body));
    ...
    console.log('Solapi response:', sendRes.status, resultText);
    const result = JSON.parse(resultText);

    if (sendRes.ok) return res.status(200).json({ success: true, data: result });
    return res.status(sendRes.status).json({ success: false, message: result.errorMessage || result.message || '발송 실패', data: result });
